...
We can see that the minimum of AIC is obtained for the ARMA(5,5) model, so the best model based on the AIC is an ARMA(5,5).

We can now fit our ARMA(5,5) model.
...
$$
X_t= 0.144X_{t-1} + 1.2499X_{t-2} - 0.6517X_{t-3} - 0.5872X_{t-4} + 0.4571X_{t-5} + 
$$
$$
Z_{t} - 0.4865Z_{t-1} - 1.5923Z_{t-2} + 0.9387Z_{t-3} + 0.8175Z_{t-4} - 0.6513Z_{t-5}
$$
...
```
The predictions (in black) are quite good. Beside the upper and lower bound of the interval of conﬁdence has big margins.
...
```
This last one is well in the interval of confidence.
