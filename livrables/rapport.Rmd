...
fic = "/Users/Acer/IdeaProjects/Air-Quality-Project/data/AirQualityUCI.csv"
...
```{r}
#We test the stationarity thanks to a Dicky-Fuller test : H0 is the hypothesis of non stationnarity. Here, the p-value is 0.01 so we reject H0 , the serie is stationary.
library(tseries)
...
Let's explain why we use this : by contrast with the AR models, it is much more dﬃcult to ﬁnd the best possible (linear) prediction of an ARMA mode. Indeed, as soon as the MA part is non degenerate, the ﬁlter can have inﬁnitely many non null coeﬃcients.
...
How does this work ?
A state space linear model of dimension r with constant coeﬃcient is given by a system of space euqation and state equations of the form : X_t = Transpose(G)Y_t +Z_T
                         Y_t = FY_{t−1} +V_t
which are respectively the Space equation and the State equation, where (Z_t) and (V_t) are uncorrelated white noise with variance R and Q, G ∈ Rr, F ∈ M(r,r) and Y ∈ Rr is the random state of the system. The Kalman theorem says : In a state-space model with constant coeﬃcients, if \widehat{Y_0} and Ω_0 are well-chosen, one can compute recursively \widehat{X_n} = Π_{n−1}(X_n), R_n = E[(X_n − \widehat{X_n})2], \widehat{Y_n} = Π_{n−1}(Y_n) and Ω_n = E[(Y_n − \widehat{Y_n})(Y_n − \widehat{Y_n}T] by the following recursion :
\widehat{Y_{n+1}} = F\widehat{Y_n} + (X_n −GT\widehat{Y_n})*FΩ_nG/(R_n)
\widehat{X_{n+1}} = transpose(G)*\widehat{Y_{n+1}}
Ω_{n+1} = FΩ_nF*transpose(F) +Q− FΩ_nG*transpose(G)*Ω_n*transpose(F)/R_n
R_{n+1} = GTΩ_{n+1}G+R
