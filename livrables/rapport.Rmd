...
We noticed that NHMC contains a lot of -200 values, so we prefered to deleted along with all the lines that contain -200. 
```{r}
...
We will build the NOx concentration a time series beacause it comes from sensor measurements, thus noise will be modeled accordingly. Since it is linearly related to the other gases (except for the temperature, relative humidity and absolute humidity that we will exclude from our study), we preferred to focus on the NOx model. 
...
Where 
$$
(S_t)_t \ the\ seasonality\  ,(T_t)_t\ the\ trend\ and\ (X_t)_t \ is\ assumed\ to\ be\ stationary  
$$
