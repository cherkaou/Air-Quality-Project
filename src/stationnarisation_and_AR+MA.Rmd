...
```{r}
#Residuals
epsilon<-ts(fit_arma_model$residuals,frequency=1,start=1)
plot.ts(epsilon)

par(mfrow=c(1,1))
acf(epsilon,lag=100)
qqnorm(epsilon)
qqline(epsilon,col=2)

par(mfrow=c(1,2))
acf(epsilon,lag=100)
acf(epsilon^2,lag=100)
...
objf.garch <- function(vartheta,epsilon,n,sig2init,petit=sqrt(.Machine$double.eps),r0=10){
  omega<-vartheta[1]
  alpha<-vartheta[2]
  beta<-vartheta[3]
  sig2<-rep(0,n)
  sig2[1]<-sig2init
  for(t in 2:n){
    sig2[t]<-omega+alpha*epsilon[t-1]^2+beta*sig2[t-1]
  }
  qml<-mean(epsilon[(r0+1):n]^2/sig2[(r0+1):n]+log(sig2[(r0+1):n]))
  qml
}
